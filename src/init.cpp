 ...
 * between arcanad, and arcana-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), ARCANA_CONF_FILENAME));
    if (mode == HMM_ARCANAD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), ARCANA_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/arcana-coin/arcana-core>";
    const std::string URL_WEBSITE = "<https://arcanacoin.org>";
...
/** Sanity checks
 *  Ensure that Arcana is running in a usable environment with all
...
{
    // Make sure only a single Arcana process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", ARCANA_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if arcana is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if arcana is started while in a temporary directory.\n",
