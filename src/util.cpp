...
const char * const ARCANA_CONF_FILENAME = "arcana.conf";
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Arcana";
#else
    // Unix
    return pathRet / ".arcana";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No arcana.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override arcana.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", ARCANA_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Arcana Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Arcana Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Arcana Core developers";
    }
