...
def main():
    ...
    enable_bitcoind = config["components"].getboolean("ENABLE_ARCANAD")
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and arcanad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if arcanad is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "arcanad"]) is not None:
            print("%sWARNING!%s There is already a arcanad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "ARCANAD" not in os.environ:
        os.environ["ARCANAD"] = build_dir + '/src/arcanad' + exeext
        os.environ["ARCANACLI"] = build_dir + '/src/arcana-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `arcana-cli help` (`rpc_interface.txt`).
