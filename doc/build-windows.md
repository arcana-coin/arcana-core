...
Below are some notes on how to build Arcana Core for Windows.

The options known to work for building Arcana Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Arcana Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Arcana Core applications.
...
Note that for WSL the Arcana Core source path MUST be somewhere in the default mount file system, for
