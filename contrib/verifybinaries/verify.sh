#!/bin/bash
# Copyright (c) 2016 The Bytcoyn Core Developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   arcanacore.org and arcanacoin.org and compares them.
...
WORKINGDIR="/tmp/arcana_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://arcanacore.org"
HOST2="https://arcanacoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="arcana-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'arcana-',
...
if [ $? -ne 0 ]; then
   echo "arcanacoin.org failed to provide signature file, but arcanacore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "arcanacoin.org and arcanacore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Arcana Core binary release signing key installed?"
   fi
