#!/bin/sh
# Copyright (c) 2014-2016 The Bytcoyn Core Developers
...
while true; do
	...
	if [ "$NO_SHA1" = "1" ]; then
		export ARCANA_VERIFY_COMMITS_ALLOW_SHA1=0
	else
		export ARCANA_VERIFY_COMMITS_ALLOW_SHA1=1
	fi

	if [ "${REVSIG_ALLOWED#*$CURRENT_COMMIT}" != "$REVSIG_ALLOWED" ]; then
		export ARCANA_VERIFY_COMMITS_ALLOW_REVSIG=1
	else
		export ARCANA_VERIFY_COMMITS_ALLOW_REVSIG=0
	fi
