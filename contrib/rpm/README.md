...
The RPM spec file provided here is for Arcana-Core 0.12.0 and builds on CentOS
...
[Arcana git](https://github.com/arcana-coin/arcana-core) but are not part of
...
Currently only `Source10` is used, for the example `arcana.conf` file.
...
in the arcana git repository. At present only an SVG version of the Arcana
icon is used.
...
needed when 0.12.1 is released, a proper fix is already in the Arcana git
master branch.
...
idea to build Arcana Core against a static version of that library compiled
...
Arcana-Core against OpenSSL or use a patched version of Boost in the build
system.
...
packaging Arcana for Fedora at least since 2012.
