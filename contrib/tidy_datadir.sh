#!/bin/bash
# Copyright (c) 2013 The Bytcoyn Core Developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Arcana database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Arcana datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Arcana datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Arcana 0.7 datadir."
    ;;
  3)
    echo "Detected Arcana pre-0.8 datadir."
    ;;
  4)
    echo "Detected Arcana 0.8 datadir."
    ;;
esac
